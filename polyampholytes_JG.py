        ...
        if score>=cutoff:
            if polyamph_start == 0:
                ...
                if extended_reg.count('R')+extended_reg.count('K')+extended_reg.count('E')+extended_reg.count('D')>=cutoff:
                    ...
                    polyamph_end = end
                    last_start = polyamph_start #without this, no seq for those with only 1 polyamph region
                    last_end = polyamph_end
            elif polyamph_end+1 > start: #if next start is within 10 from last end, join the 2
                if extended_reg.count('R')+extended_reg.count('K')+extended_reg.count('E')+extended_reg.count('D')>=cutoff:
                    polyamph_end = end
                    last_start = polyamph_start #without this, no seq for last polyamph region
                    last_end = polyamph_end
            else:
                if extended_reg.count('R')+extended_reg.count('K')+extended_reg.count('E')+extended_reg.count('D')>=cutoff:
